...
async function run() {     
  ...
  await page.evaluate(()=>{
    ...
    // Click on Solid Logo
    document.querySelectorAll("#footer .swatch")[5].click()
  })
  ...
  for(let i=0;i<colorLengths.color1Length;i++)
  {
    ...
    // Find lengths of Colors available
    let color2Length = await page.evaluate(()=>{
      let color2Length = document.querySelectorAll("#debug-189 .swatch").length;
      return color2Length
    })
    colorLengths["color2Length"] = color2Length
    console.log(colorLengths)
    ...
    for(let j=0; j<colorLengths.color2Length; j++)
    {
      ...
      await page.screenshot({
        path: `./Full-Fade-Images/full-fade-${i+1}-${j+1}.jpg`,
        type: "jpeg",
        fullPage: true
      });

      console.log(`full-fade-${i+1}-${j+1}.jpg`)
    }
  }
